'''
Ethernet learning switch in Python.

Note that this file currently has the code to implement a "hub"
in it, not a learning switch.  (I.e., it's currently a switch
that doesn't learn.)
'''
from switchyard.lib.userlib import *
import sys
table=[]
MAX_LEN=2
def switch_table_get(src_mac):
    for key in table:
        if key[0]==src_mac:
            return key
    return None
def switch_table_get_i(src_mac):
    for i,key in enumerate(table):
        if key[0]==src_mac:
            return i
    return None
def switch_table_insert(port_info):
    table.append(port_info)
def learning(input_port,src_mac):
    port_info=switch_table_get(src_mac)
    if port_info is None:#src_mac not in the table
        if len(table)<MAX_LEN:#table not full, just add
                switch_table_insert([src_mac,input_port,0])
        else:#table full,do something
            min=sys.maxsize
            index=0
            for i,key in enumerate(table):
                if min>key[2]:
                    min=key[2]
                    index=i
            del table[index]
            switch_table_insert([src_mac,input_port,0])
            
    else:#src_mac is in the forwarding table
        if input_port==port_info[1]:#topology not changed do nothing
            return
        else:#topology changed , change the port, do not change the tracffic_info
            i=switch_table_get_i(src_mac)
            table[i][1]=input_port        
    
    log_info("\nmy_momory: {}\n".format(table))
def Flood_packet(net,my_interfaces,input_port,packet):
    for intf in my_interfaces:
        if input_port!=intf.name:
            log_info("Flooding packet {} to {}".format(packet,intf.name))
            net.send_packet(intf.name,packet)
            
def Transport_packet(net,my_interfaces,input_port,packet):
    eth=packet.get_header(Ethernet)
    port_info=switch_table_get(eth.dst)
    if port_info is None:#dst is not in table , flood
        Flood_packet(net,my_interfaces,input_port,packet)
    else:#dst in table ,update the time
        log_info("Transport packet {} to {}".format(packet,port_info[0]))
        i=switch_table_get_i(eth.dst)
        table[i][2]=table[i][2]+1
        net.send_packet(port_info[1],packet)
        log_info("memory after transport: {}".format(table))
    
def main(net):
    my_interfaces = net.interfaces() 
    mymacs = [intf.ethaddr for intf in my_interfaces]

    while True:
        try:
            timestamp,input_port,packet = net.recv_packet()
        except NoPackets:
            continue
        except Shutdown:
            return

        log_info ("In {} received packet {} on {}".format(net.name, packet, input_port))
        eth=packet.get_header(Ethernet)
        
        learning(input_port,eth.src)
        
        if eth is None:
            log_info("Received a non-Ethernet packet?!")
            continue
        if eth.dst in mymacs:
            log_info("Packet intended for me")
        else:
            Transport_packet(net,my_interfaces,input_port,packet)
        '''if packet[0].dst in mymacs:
            log_debug ("Packet intended for me")
        else:
            for intf in my_interfaces:
                if input_port != intf.name:
                    log_debug ("Flooding packet {} to {}".format(packet, intf.name))
                    net.send_packet(intf.name, packet)'''
    net.shutdown()
